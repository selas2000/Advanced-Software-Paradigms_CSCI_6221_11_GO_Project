...
import (
	...
	"github.com/selas2000/Advanced-Software-Paradigms_CSCI_6221_11_GO_Project/eCommerce/database/mongodb"
	"github.com/selas2000/Advanced-Software-Paradigms_CSCI_6221_11_GO_Project/eCommerce/database/redis"
	"github.com/selas2000/Advanced-Software-Paradigms_CSCI_6221_11_GO_Project/eCommerce/models"
)
	...
	if tokenFound && credFound && mongodbDataFound && RefreshToken(w, r) {
		if r.Method == "GET" {
			note := models.SignupData{
				...
				Lname:    mongodbData[1],
				Email:    mongodbData[2],
				Password: mongodbData[3], // Plaintext password here
				Phone:    mongodbData[4],
...
// replace the old info with the newly entered info in the database. After that,
// it will delete the old credentials and set the new credentials and new token
// to automatically login so that a user doesn't have to login again.
		...
		if r.Method == "GET" {
			...
			if r.FormValue("update-profile") == "UpdateProfile" {
				...
				if pData.Password == pData.ConfirmPassword {
					newCred := [11]string{pData.Fname, pData.Lname, pData.Email, pData.Password, pData.Phone, pData.Country, pData.State, pData.Faddress, pData.Laddress, pData.Postal, pData.Company}
					// mysql.UpdateProfile(newCred, credData)
					mongodb.UpdateProfile(newCred, credData)
					redis.Del("LoginPass")
					redis.SetCredentials("LoginPass", newCred[2], newCred[3]) // Set plaintext password
					// mysqlDataFound, _ := mysql.FindAccount(newCred[2], newCred[3])
					mongodbDataFound, _ := mongodb.FindAccount(newCred[2], newCred[3])
					if mongodbDataFound && newCred[3] == pData.Password { // Direct comparison of passwords
...
func DeleteProfile(w http.ResponseWriter, r *http.Request) error {
	...
	if tokenFound && credFound && mongodbDataFound && RefreshToken(w, r) {
		if r.Method == "GET" {
			...
			if credData[1] == findPass.Password { // Direct comparison of passwords
